...
| **Editable name-based exclusions** | `Config.exclude_patterns` lists surface-name patterns to *ignore* entirely (never parsed, fit, sewn, or written). It defaults to `("Pylon_heatshield_body",)` — dropping the heatshield body that otherwise sews as its own contact-only shell around the core. Edit the tuple in `config.py` to add/remove surfaces, or override per-run: `--exclude PATTERN` (repeatable, adds), `--no-exclude` (include everything), or `RB3135_EXCLUDE="patA,patB"` / `RB3135_EXCLUDE=none`. A pattern is a case-insensitive **substring** unless it contains a glob metacharacter (`* ? [`), in which case it is matched with `fnmatch` against the full file stem. |
